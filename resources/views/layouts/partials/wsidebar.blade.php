<aside id="sidebar" class="bg-white border-r border-gray-200 w-64 fixed inset-y-0 left-0 transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out z-30 overflow-y-auto">
  ...
  <div class="flex flex-col h-full">
      ...
      <nav class="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
          ...
          <a href="{{ route('worker.notifications') }}" class="flex items-center px-2 py-2 text-sm font-medium rounded-md {{ request()->is('worker/notifications*') ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                <svg class="mr-3 h-5 w-5 {{ request()->is('worker/notifications*') ? 'text-blue-500' : 'text-gray-400' }}" 
